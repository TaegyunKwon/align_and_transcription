...
tf.app.flags.DEFINE_integer('seg_length', 120, 'segment length')
tf.app.flags.DEFINE_integer('overlap', 20, 'overlap length')

tf.app.flags.DEFINE_string('mode', 'align',
                           'dataset_mode')
...
def generate_test_set():
  ...
  '''
  wav_files = find_files_in_subdirs(FLAGS.input_dir, '*.mp3')
  wav_files.extend(find_files_in_subdirs(FLAGS.input_dir, '*.wav'))
  for wav_file in wav_files:
      base_name_root, _ = os.path.splitext(wav_file)
      mid_file = base_name_root + '.mid'
      test_file_pairs.append((wav_file, mid_file))
  ...
  with tf.python_io.TFRecordWriter(test_output_name) as writer:
    for pair in test_file_pairs:
      ...
      splits = find_split_points_midi_only(ns, samples, FLAGS.sample_rate,
                                 FLAGS.min_length, FLAGS.max_length)
      if splits is None:
        f = open(os.path.join(FLAGS.output_dir, 'not_seg.txt'), 'wb')
        f.write(pair[0] + '/')
        f.close()
      else:
          n_seg = 0
...
def generate_align_set():
  ...
  with tf.python_io.TFRecordWriter(test_output_name) as writer:
    for wav_file in wav_files:
      print(wav_file)
      # load the wav data and resample it.
      samples = audio_io.load_audio(wav_file, FLAGS.sample_rate)
      wav_data = audio_io.samples_to_wav_data(samples, FLAGS.sample_rate)

      len_wav = len(samples) / FLAGS.sample_rate
      n_seg = int(np.ceil((len_wav - FLAGS.seg_length) / (FLAGS.seg_length - FLAGS.overlap))) + 1

      splits = []
      gap = FLAGS.seg_length - FLAGS.overlap
      for n in range(n_seg - 1):
          splits.append((n * gap, n * gap + FLAGS.seg_length))
      splits.append(((n_seg - 1) * gap, len_wav))
      ...
      for start, end in splits:
        ...
        id = wav_file + '_seg{:d}'.format(n_seg)
...
def find_split_points_midi_only(note_sequence, samples, sample_rate, min_length,
                      max_length):
  """Returns times at which there are no notes.
  ...
  while end_time - split_points[-1] > max_length:
    ...
    # if no interval, or we didn't fit, try the unmodified sequence
      ...
      if pos < len(nosustain_starts) and max_advance > nosustain_starts[pos]:
        ...
        print('no possible split found. split_points:{}'.format(split_points))
