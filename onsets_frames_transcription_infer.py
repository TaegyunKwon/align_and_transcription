  ...
  with tf.Graph().as_default():
    ...
    with tf.train.MonitoredSession(session_creator=session_creator) as sess:
      ...
      for unused_i in range(acoustic_data_provider.num_batches):
        ...
        sequence_label, save_dict = infer_util.score_sequence(
            sess,
        ...
        np.save(os.path.join(run_dir, filename + '_labels.npy'), labels)
        np.save(os.path.join(run_dir, filename + '_logits.npy'), logits)
        np.save(os.path.join(run_dir, filename + '_onsets.npy'), onset_logits)

        with open(os.path.join(run_dir, filename + '_summary.pkl'), 'wb') as f:
